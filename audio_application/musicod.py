...
def album_od(lib_data, lib_dir, inlier_albums, outlier_albums=[], output='out.csv', weighted=True):
    ...
    if weighted:
        ...
        clf.threshold_ = np.percentile(
            clf.decision_scores_, 100 * (1 - clf.contamination))
        clf.labels_ = (clf.decision_scores_ >
                       clf.threshold_).astype('int').ravel()
...
if __name__ == '__main__':
    ...
    album_od(lib_data, itunes_dir, [
             'Nami Haven/Void Ocean／Heavy Rain'], output='out3.csv', weighted=True)
