...
train = pd.read_csv("./naive/naive_test.csv",encoding='unicode_escape', error_bad_lines=False)
train.columns = ['A','B']
# test = pd.read_csv("./naive/naive_test.csv",encoding='unicode_escape')
# test.columns = ['A','B']
# print(datas['F'][1])

train_model = TfidfVectorizer().fit(train['B'])
train_tfidf = train_model.transform(train['B'])     # 得到tf-idf矩阵，稀疏矩阵表示法
...
# test_model = TfidfVectorizer().fit(test['B'])
# test_tfidf = train_model.transform(test['B'])     # 得到tf-idf矩阵，稀疏矩阵表示法
